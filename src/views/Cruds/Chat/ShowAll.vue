        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- Start:: Status Input -->
              <base-input type="text" col="12" :placeholder="$t('PLACEHOLDERS.name_requester')"
      ...
      <v-data-table class="thumb" :loading="loading" :loading-text="$t('TABLES.loadingData')" :search="searchValue"
        ...
        <template v-slot:[`item.avatar`]="{ item }">
          <div class="table_image_wrapper">
            <h6 class="text-danger" v-if="!item.avatar"> {{ $t("TABLES.noData") }} </h6>

            <button class="my-1" @click="showImageModal(item.avatar)" v-else>
              <img class="rounded" :src="item.avatar" :alt="item.name" width="60" height="60" />
            </button>
          </div>
        </template>
        <!-- End:: Item Image -->
        ...
        <template v-slot:[`item.LastMessage`]="{ item }">
          <template>
            <h6 class="text-danger" v-if="item.LastMessage.length === 0"> {{ $t("TABLES.noData") }} </h6>
            <div class="actions" v-else>
              <button class="btn_show" @click="showReplayModal(item.LastMessage)">
...
export default {
  ...
  data() {
    return {
      ...
      tableHeaders: [
        {
          text: this.$t("TABLES.TABLES.serialNumber"),
          value: "id",
          sortable: false,
          align: "center",
        },
        ...
        {
          text: this.$t("PLACEHOLDERS.name_requester"),
          value: "name",
          sortable: false,
          align: "center",
        },
        {
          text: this.$t("PLACEHOLDERS.LastMessage"),
          value: "LastMessage",
          sortable: false,
          align: "center",
        },
        {
          text: this.$t("PLACEHOLDERS.LastMessageDate"),
  ...
  methods: {
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            mine: 1,
            search: this.filterOptions.status,
          },
        });
        this.loading = false;
    ...
    // ===== Start:: Show
    showItem(item) {
      this.$router.push({ path: `/chat/show/${item.id}` });
    },
    // ===== End:: Show
