<template>
  <div class="crud_form_wrapper">
    ...
    <div class="form_title_wrapper">
      <h4>{{ $t("PLACEHOLDERS.show_equipment") }}</h4>
    </div>
    <!-- End:: Title -->
      ...
      <form @submit.prevent="validateFormInputs">
        <div class="row">
          ...
          <base-input col="6" type="number" v-if="data.is_payment.value == 1 || data.is_payment.id == 1"
            :placeholder="$t('PLACEHOLDERS.min_payment')" v-model.trim="data.min_payment" disabled />

          <base-input col="6" type="number" v-if="data.is_payment.value == 1 || data.is_payment.id == 1"
            :placeholder="$t('PLACEHOLDERS.day_payment')" v-model.trim="data.day_payment" disabled />

          <base-select-input col="6" :optionsList="allTransport" :placeholder="$t('PLACEHOLDERS.transport_type')"
            v-model="data.transport_type_id" disabled />


          <base-select-input col="6" :optionsList="main_cat" :placeholder="$t('PLACEHOLDERS.main_cat')"
            v-model="main_list" disabled @input="getSub(main_list)" />

          <base-select-input col="6" v-if="sub_cat.length || main_list" :optionsList="sub_cat"
            :placeholder="$t('PLACEHOLDERS.sub_cat')" v-model="sub_list" disabled @input="get_inner_Sub(sub_list)" />

          <base-select-input col="6" v-if="sub_inner_cat.length || sub_list" :optionsList="sub_inner_cat"
            :placeholder="$t('PLACEHOLDERS.inner_sub_cat')" v-model="sub_inner_list" disabled @input="GetFeatures" />
          ...
          <div class="input_wrapper top_label d-none">
